...
import type { ExperienceType, WorkHistoryItem } from '../../../model';

const TYPE_CLASSES: Record<ExperienceType, string> = {
	frontend: 'text-[#8B5CF6] bg-[#8B5CF6]/10',
	fullstack: 'text-[#10B981] bg-[#10B981]/10',
	other: 'text-[#99a1af] bg-[#99a1af]/10',
};
...
export const ExperienceCard = ({ experience }: { experience: WorkHistoryItem }) => {
	const { role, company, period, summary, highlights, type } = experience;

	return (
		<article className="card-surface flex flex-col gap-3 lg:p-6!">
			<header className="flex flex-col gap-2 lg:gap-3">
				<div className="flex flex-col items-baseline gap-3">
					<span className={cn('rounded-lg px-3 py-1 text-xs font-semibold uppercase', TYPE_CLASSES[type])}>
						{TYPE_LABELS[type]}
					</span>
